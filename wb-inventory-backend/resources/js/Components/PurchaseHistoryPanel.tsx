...
import type { PurchaseProps } from "@/Pages/Reports/types";
...
export function PurchaseHistoryPanel({ products, purchaseHistories, costSummaries }: PurchaseProps) {
  const [sku, setSku] = useState(products[0]?.sku ?? "");
  const history = purchaseHistories[sku] ?? [];
  const cost = costSummaries[sku] ?? { currentCost: 0, lastPurchaseCost: null, lastPurchaseDate: null, averageCost: null, trend: null };
  ...
  return (
    <div className="card-surface overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Every receipt for a SKU — supplier, quantity, date, price</p>
          </div>
        </div>
        <select
          aria-label="Purchase history SKU"
          value={sku}
      ...
      {history.length === 0 ? (
        <p className="py-10 text-center text-sm text-muted-foreground">No receipts on record for this SKU yet.</p>
      ) : (
        <>
