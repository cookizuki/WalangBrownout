...
import { money } from "@/Components/PODraftPreviewModal";
...
export default function ReportsPage({ shrinkageByMonth, velocityBySku, valuation, turnover, deadStock, supplierPerf, products, purchaseHistories, costSummaries }: ReportProps) {
  const c = { '--abc-a': '#2563eb', '--abc-b': '#d97706', '--abc-c': '#64748b', '--border': '#d1d5db', '--muted-foreground': '#64748b', '--foreground': '#111827', '--danger': '#dc2626', '--success': '#16a34a', '--warning': '#d97706' };
  const totalTiedUp = deadStock.reduce((sum, row) => sum + row.tiedUpValue, 0);
  const totalShrinkageCost = shrinkageByMonth.reduce((sum, row) => sum + row.cost, 0);
  const abcColor = { A: c["--abc-a"], B: c["--abc-b"], C: c["--abc-c"] } as const;

  return (
    <div id="reports-print-area" className="mx-auto max-w-7xl space-y-6 p-4 sm:p-8"><Head title="Reports" />
      ...
      <div className="space-y-2">
        ...
        <PurchaseHistoryPanel products={products} purchaseHistories={purchaseHistories} costSummaries={costSummaries} />
      </div>
    </div>
  );
}